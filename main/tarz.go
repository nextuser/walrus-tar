...
func TarWrite(src string, w io.Writer) (err error) {
	// 创建 Tar.Writer 结构
	tw := tar.NewWriter(w)
	...
	return filepath.Walk(src, func(fileName string, fi os.FileInfo, err error) error {
		...
		// 例如： 将原本 hdr.Name 的 syslog 替换成 log/syslog
		...
		debug("walk filename:", fileName)

		debug("walk hdr.name:", hdr.Name)
		// 写入文件信息
		if err := tw.WriteHeader(hdr); err != nil {
			debug("write header fail", err)
			return err
		}
...
func TarFile(src string, dst string) (err error) {
	debug("tar file ", dst)
	// 创建文件
	...
	gw := bufio.NewWriter(fw)
	return TarWrite(src, gw)
